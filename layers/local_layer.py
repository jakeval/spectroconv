...
class DepthwiseLocallyConnected(nn.Module):
    def __init__(self, in_shape, in_channels, kernel_size=3, stride=1, padding=0, bias=True):
        super(DepthwiseLocallyConnected, self).__init__()
        self.k = self._to_tuple(kernel_size)
        self.c = in_channels
        self.stride = self._to_tuple(stride)
        if padding == 'same':
        ...
        K = self.k[0] * self.k[1] * self.c
        self.weight = nn.Parameter(torch.ones((L, K), dtype=torch.float64), requires_grad=True)
        if bias:
            self.bias = Parameter(torch.empty((self.c, L), dtype=torch.float64), requires_grad=True)
        else:
            self.register_parameter('bias', None)
        nn.init.kaiming_normal_(self.weight, mode='fan_out', nonlinearity='relu')
        if self.bias is not None:
    ...
    def forward(self, x):
        ...
        x_unf = F.unfold(x, k, padding=padding, stride=stride).transpose(1, 2)
        out_unf = (x_unf * self.weight).sum(-1)
        N = x.shape[0]
        out_f = F.fold(out_unf.view(N, 1, h_out*w_out), (h_out, w_out), (1, 1))
        if self.bias is not None:
            out_f = out_f + self.bias.view(-1, out_f.shape[2], out_f.shape[3])
        return out_f
